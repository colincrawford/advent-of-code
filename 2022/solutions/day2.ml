...
let input_to_outcome (input:char) =
  ...
  | 'Y' -> Draw
  | 'Z' -> Win
  | _ -> raise (InvalidInput ("Invalid input for expected outcome: " ^ (String.of_char input)))
...
let input_to_moves (input: char list list): (choice * choice * outcome * choice) list =
...
let part1 moves = moves
  |> List.map ~f:(fun (their_choice, my_choice, _, _) -> score_choices my_choice their_choice)
  |> sum_list
  |> Int.to_string

let part2 moves = moves
  |> List.map ~f:(fun (their_choice, _, _, my_choice) -> score_choices my_choice their_choice)
  |> sum_list
  |> Int.to_string

let solve : Day_solution.solver = fun input ->
